...
abnormalities, no `[N]` cell count on the karyotype line at all).
`find_candidate_iscn_lines()` returned `"46,XX ; FEMALE
KARYOTYPEResults:"`, which then failed with `'XX ; FEMALE
KARYOTYPEResults:' is not a valid sex chromosome constitution`.
Confirmed against the real PDF's own `extract_text()` output: same
report-software family as task 22/26 (also emits `"XX-XXXXXXXCust.
Specimen ID:"`, `"01/06/2016Collection Date:"` on this document), same
"value glued directly onto Label:" quirk — just landing on a karyotype
